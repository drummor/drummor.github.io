...
|             | 旧                                                                              | 新                                                                                                      |
| :---------- | :------------------------------------------------------------------------------ | :------------------------------------------------------------------------------------------------------ |
| Bridge 通信 | ![ReactNative旧版Bridge通信架构图：JS线程与Native线程通过Bridge异步通信](/images/img_20250907164052914.png)                       | ![ReactNative新版JSI架构图：JS引擎直接扩展实现同步通信](/images/img_20250907164218660.png)                                              |
| JS 引擎     | ![旧版JSC引擎运行期解析执行架构图](/images/img_20250907165242982.png)<br>解析执行是在运行期 | ![Hermes引擎支持二进制加载，解析编译前置到打包期](/images/img_20250907165330095.png)<br>引擎支持二进制加载，解析编译放在了打包期 |
...
![Hippy动态化跨端三层架构范式图：开发期、解析执行期、渲染期](/images/Pasted%20image%2020250905152117.png)hippy 已经把动态化跨端给范式化了，分为开发期、解析执行期、渲染期。
...
![Hummer跨端框架架构图：JS引擎、JSBridge、YogaLayout、NativeRender四层结构](/images/Pasted%20image%2020250903175541.png)
...
![Lynx早期架构图：DOM节点构建在Native层，JS运行业务逻辑不阻塞UI展示](/images/Pasted%20image%2020250903175600.png)
...
![Lynx轻JS逻辑设计图：首屏直出，DOM节点Native构建，JS执行业务逻辑](/images/Pasted%20image%2020250905152041.png)
- lynx 最重要的渲染点在于首屏直出。Lynx 是将 DOM 节点构建全部放在 Native （C++）层。JS 运行的是业务逻辑、不阻塞 UI 展示，整体轻 JS 逻辑设计；
![Lynx自建渲染工具优化架构图：扩展平台便捷及针对性优化](/images/img_20250907171732372.png)
...
**项目架构**
![MTFlexBox项目架构图：驱动层、DOM层、渲染层三层结构](/images/Pasted%20image%2020250905151953.png)
核心流程
![MTFlexBox核心渲染流程图：异步解析、缓存、Layout计算](/images/Pasted%20image%2020250905151959.png)
优化思路
...
![DynamicX渲染流程图：协议二进制解析、属性转换、原生控件渲染](/images/Pasted%20image%2020250905151543.png)
...
![GaiaX跨端模板引擎架构图：Layout、CSS、Data、JS四协议分层](/images/Pasted%20image%2020250905151751.png)
...
![Cube卡片技术栈架构图：JSEngine、CardEngine、RenderEngine、Platform四层](/images/Pasted%20image%2020250905151817.png)
...
![Jetpack Compose技术分层架构图：Compiler、Runtime、UI、Foundation、Material五层](/images/Pasted%20image%2020250904140313.png)
...
![Compose Compiler代码生成与Runtime执行流程图：Composable函数到Slot Table状态树](/images/Pasted%20image%2020250904115140.png)
...
![RedWood跨平台原理图：状态树到各平台渲染树映射机制](/images/Pasted%20image%2020250904115157.png)
