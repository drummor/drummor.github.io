...
-   消息堆栈：对消息内具体执行信息，采集其堆栈。 ![今日头条Raster消息调度监控架构图：历史消息、正在执行消息、待执行消息三类采集](/images/big_factory_apm_total1706189948025.png)

#### 2.3 主线程线程堆栈的采样

对耗时的消息，进行采样，采取的策略是超时采样。具体来说，介于大部分的耗时小消息不需要进行堆栈采样，为了避免频繁设置和取消超时任务（也就是采样任务），头条在此处做了一个优化，每次消息开始是并不是重新设置采集超时任务，而是修改目标时间。 ![头条主线程堆栈超时采样策略图：修改目标时间而非重新设置采样任务优化](/images/big_factory_apm_total1706189948091.png)
...
![微信WatchDog方案漏报概率分析图：4.5秒检测周期下5秒耗时任务仅11%检测概率](/images/big_factory_apm_total1706189948136.png)
...
![钉钉ANR监控主线程调度分类图：消息调度、IdleHandler、IDLE状态、触摸事件、传感器事件五类](/images/big_factory_apm_total1706189948174.png)
...
![业界ANR监控方案总结架构图：SIGQUIT感知、消息调度采集、堆栈采样三层次](/images/big_factory_apm_total1706189948248.png)
